...
sub control_component_reboot {
    ...
    $cmd = "$runat ssh -o StrictHostKeyChecking=no root\@$ip 'hostname > /etc/hostname'";
    my ($rc, $crc, $buf) = piperun($cmd, "", "ubero");
...
sub describe_services {
	...
	if ($rc || $crc) {
	    ...
	    $cmd = "$runat $remote_pre if netstat -tan | grep 8443 >/dev/null ; then euca-describe-services; else exit 1; fi $remote_post";
	    ($crc, $rc, $buf) = piperun($cmd, "", "ubero");
	    if ($crc || $rc) {
...
sub stop_instance{
    ...
    $cmd = "$runat $prefix-stop-instances $inst";
    ($crc, $rc, $buf) = piperun($cmd, "", "$ofile");
    if ($rc || $crc) {
    	doexit(1, "FAILED: ($crc, $rc, $buf) = $cmd\n");
    }
...
sub start_instance{
    ...
    $cmd = "$runat $prefix-start-instances $inst";
    ($crc, $rc, $buf) = piperun($cmd, "", "$ofile");
    if ($rc || $crc) {
    	doexit(1, "FAILED: ($crc, $rc, $buf) = $cmd\n");
    }
